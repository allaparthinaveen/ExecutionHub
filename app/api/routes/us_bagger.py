...
logger = logging.getLogger("tradeservices.routes.us_bagger")
router = APIRouter()

@router.get("/tickers", response_model=List[str])
def get_us_tickers(user_id: str = Depends(get_current_user)):
    """
    Fetch the list of active US listed equities symbols.
    Downloads the active register from the public registry.
    """
    try:
        symbols = BaggerScannerService.fetch_us_symbols()
        return symbols
    except Exception as e:
        logger.error(f"Error fetching active US symbols: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to fetch active US symbols register: {str(e)}"
        )
...
async def scan_tickers(
    ...
    """
    Scan US tickers for 100-bagger potential.
    
    If 'tickers' list is empty and 'use_db' is True:
      - Instantly queries the pre-computed US database results table.
    ...
    if not tickers_list and request.use_db:
        logger.info(f"User {user_id} requested database US scan read.")
        try:
            ...
            total_db_records = db.query(USBaggerScanResult).count()
            total_passed = db.query(USBaggerScanResult).filter(USBaggerScanResult.score >= 90.0).count()
            total_failed = db.query(USBaggerScanResult).filter((USBaggerScanResult.score < 90.0) & (USBaggerScanResult.label != "Insufficient Data")).count()
            total_insufficient = db.query(USBaggerScanResult).filter(USBaggerScanResult.label == "Insufficient Data").count()
            ...
            logger.error(f"Database US scan query failed: {db_err}")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    if not tickers_list:
        logger.info(f"Empty US tickers list received; fetching active symbols registry to auto-limit {request.auto_limit} symbols.")
        try:
            full_list = BaggerScannerService.fetch_us_symbols()
            tickers_list = full_list[:request.auto_limit]
        except Exception as e:
            logger.error(f"Failed to fetch US symbols list for auto-scan: {e}")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Auto-scan failed to retrieve active US symbols registry: {str(e)}"
            )
            
    if not tickers_list:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No tickers provided and active US symbols registry is empty."
        )
 
    logger.info(f"Running live US 100-bagger screening for {len(tickers_list)} symbols in parallel.")
    try:
        ...
        logger.error(f"US 100-bagger scanning execution failed: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def trigger_scan_job(
    ...
    """
    Trigger the daily background US 100-bagger scanning job.
    Runs asynchronously using FastAPI BackgroundTasks to prevent HTTP timeouts.
    """
    logger.info(f"User {user_id} triggered background US scan job with limit={limit}.")
    background_tasks.add_task(
        BaggerScannerService.run_background_scan,
        limit=limit,
        market="US"
    )
    return {"message": "Background US scanning job triggered successfully. View console logs for details."}
...
async def run_portfolio_backtest(
    ...
    """
    Backtest the US High Potential candidates currently stored in the database.
    
    Simulates investing a fixed amount (default: $50,000) split equally among
    all passed US candidates starting at the specified start year, and calculates
    individual and overall annualized returns (CAGR) and final valuations.
    """
    logger.info(f"User {user_id} requested US portfolio backtest starting from {start_year} (investment: {total_investment}).")
    
    # 1. Fetch High Potential candidates (passed candidates)
    records = db.query(USBaggerScanResult).filter(
        (USBaggerScanResult.passed == True) | (USBaggerScanResult.label == "High Potential")
    ).all()
    
    if not records:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="No High Potential US candidates found in the database. Please run a scan job first to populate results."
        )
    ...
    logger.info(f"Downloading bulk price data from yfinance for {len(tickers)} US tickers...")
    try:
        ...
        logger.error(f"Failed to fetch bulk price data for US stocks: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    for ticker in tickers:
        ...
        if not price_then or not price_now or pd.isna(price_then) or pd.isna(price_now):
            logger.warning(f"Missing price data for US ticker {ticker}. Skipping from portfolio calculations.")
            continue
    ...
    if not candidates_results:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to resolve price data for any of the High Potential US candidates."
        )
