...
-- Create table for US 100-Bagger scanner results
CREATE TABLE IF NOT EXISTS us_bagger_scan_results (
    id SERIAL PRIMARY KEY,
    ticker VARCHAR UNIQUE NOT NULL,
    company_name VARCHAR,
    passed BOOLEAN DEFAULT FALSE,
...
CREATE INDEX IF NOT EXISTS ix_us_bagger_scan_results_ticker ON us_bagger_scan_results (ticker);
CREATE INDEX IF NOT EXISTS ix_us_bagger_scan_results_score ON us_bagger_scan_results (score);
